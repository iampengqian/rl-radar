...
async function main(): Promise<void> {
  ...
  // Save RL and Orch reports
  console.log(
    `  Saved ${saveRlDailyReport(rlDigests, rlComparison, utcStr, dateStr, autoGenFooter("zh"), "zh")}`,
  );
  console.log(
    `  Saved ${saveOrchDailyReport(orchDigests, orchComparison, utcStr, dateStr, autoGenFooter("zh"), "zh")}`,
  );
  console.log(
    `  Saved ${saveRlDailyReport(enRlDigests, enRlComparison, utcStr, dateStr, autoGenFooter("en"), "en")}`,
  );
  console.log(
    `  Saved ${saveOrchDailyReport(enOrchDigests, enOrchComparison, utcStr, dateStr, autoGenFooter("en"), "en")}`,
  );
  ...
  if (digestRepo) {
    ...
    // Create Orch issues
    const orchContent = buildOrchReportContent(
      orchDigests,
      orchComparison,
      utcStr,
      dateStr,
    ...
    const orchUrl = await createGitHubIssue(
      ORCH_ISSUE_TITLE(dateStr, "zh"),
      orchContent,
      ISSUE_LABELS.orch.zh,
    );
    console.log(`  Created Orch issue (zh): ${orchUrl}`);

    const orchEnUrl = await createGitHubIssue(
      ORCH_ISSUE_TITLE(dateStr, "en"),
      enOrchContent,
      ISSUE_LABELS.orch.en,
    );
